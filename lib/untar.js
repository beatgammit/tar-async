	...
	Untar.prototype.write = function write(data, encoding) {
		...
		doHeader.call(this, tBuf, function (err, data, rOffset) {
			...
			if (buffer.length >= data.fileSize) {
				fileStream.emit('data', buffer.slice(0, data.fileSize));
				fileStream.emit('end');
				totalRead += data.fileSize;
				buffer = buffer.slice(data.fileSize);
			...
			leftToRead = data.fileSize - buffer.length;
